...
if (validationForm != null) {
  validationForm.addEventListener("submit", async (el) => {
    el.preventDefault();
    try {
      const response = await fetch(`${ipClient}/api/utilizador/confirmacaoconta`, {
        method: 'POST',
...
if (loginForm != null) {
  ...
  loginForm.addEventListener("submit", async (element) => {
    ...
    try {
      const response = await fetch(`${ipClient}/api/utilizador/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
if (recoverForm != null) {
  recoverForm.addEventListener("submit", async (element) => {
    element.preventDefault();
    try {
      const response = await fetch(`${ipClient}/api/utilizador/${localStorage.getItem("_")}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          senha: senhaField.value
...
if (loginBtn !== null) {
  loginBtn.addEventListener("click", async () => {
    try {
      const response = await fetch(`${ipClient}/api/utilizador/authenticate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: emailField.value,
...
if (recuperarBtn !== null) {
  recuperarBtn.addEventListener("click", async () => {
    try {
      const response = await fetch(`${ipClient}/api/utilizador/recover`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: emailField.value,
...
if (validaCodeBtn !== null) {
  validaCodeBtn.addEventListener("click", async () => {
    try {
      const response = await fetch(`${ipClient}/api/utilizador/recovervalidation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: JSON.parse(localStorage.getItem("currentSession")).email,
...
if (ibanField !== null) {
  ibanField.addEventListener('focus', () => {
    ibanField.value = "AO06.";
  });
}

if (ibanField !== null) {
  ibanField.addEventListener('keyup', (el) => {
    const ibanValue = (el.currentTarget.value).slice(5);
    var value = ibanValue.replace(/\D/g, ''); // Remove non-numeric characters
    var formattedValue = '';
    for (var i = 0; i < value.length; i++) {
    ...
    ibanField.value = 'AO06.' + formattedValue;
  });
}
